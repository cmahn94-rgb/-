...
def analyze_one(종목, settings):
    ...
    if 결과["매수신호"]:
        ...
        # Sharpe 0.5 미만 → 백테스트 품질 불량 → 매수신호 무효화
        # Sharpe 0.5 미만 = 위험 대비 수익이 너무 낮아 신뢰 불가
        MIN_SHARPE = float(settings.get("MIN_SHARPE", 0.5))
        if bt1y is not None and bt1y["sharpe"] < MIN_SHARPE:
            결과["매수신호"] = False  # 매수 신호 무효화
            print(f"  ⛔ {name} 백테스트 Sharpe {bt1y['sharpe']:.2f} < {MIN_SHARPE} → 매수신호 차단")
...
def build_report_sections(종목목록, settings, 크립토_하락_레짐, 포트폴리오=None):
    ...
    for 종목 in 종목목록:
        ...
        # 갭업 1~3% 구간은 우선순위 점수 패널티 적용
        # (기존 2%→1%: 경계값 버그 수정 + 패널티 구간 확대)
        if 갭 >= 1.0:
            갭_패널티 = round((갭 - 1.0) * 5, 1)  # 1%→0점, 3%→10점 패널티
            우선순위점수 = max(0, 우선순위점수 - 갭_패널티)
        # 갭다운은 분할매수 전략이므로 패널티 없음 (경고만)
