...
def analyze_one(종목, settings):
    ...
    if 결과["매수신호"]:
        ...
        if wf is not None:
            ...
            # 문제3 수정: 검증 구간 거래 4회 미만이면 차단 보류
            # 거래가 너무 적으면 Sharpe가 -80 같은 비정상 값으로 왜곡됨
            # (펩시코 -80.23, T모바일 -68.14 등 → 표본 부족이 원인)
            if wf["과적합_경고"] and 검증_결과 is not None:
                val_sh   = 검증_결과.get("sharpe", 0)
                val_거래 = 검증_결과.get("거래횟수", 0)
                if val_거래 < 4:
                    # 거래 부족 → Sharpe 신뢰 불가 → 차단하지 않고 주의만 표시
                    bt_문구 += f"  ⚠️ WF 검증 거래 부족({val_거래}회) → 차단 보류, 참고용\n"
                elif val_sh < 0.3:
                    결과["매수신호"] = False
                    bt_문구 += f"  ⛔ WF 과적합 + 검증Sharpe {val_sh:.2f} < 0.3 → 매수신호 차단\n"
                    print(f"  ⛔ {name} WF과적합 차단 (Sharpe {val_sh:.2f}, 거래 {val_거래}회)")
...
def build_report_sections(종목목록, settings, 크립토_하락_레짐, 포트폴리오=None):
    ...
    _workers = 8 if _os_tmp.getenv("DART_API_KEY") else MAX_WORKERS
    ...
    for rank, (ticker, 우선순위점수, 우선순위세부) in enumerate(신호_종목_순서, 1):
        ...
        if fa:
            ...
            if fa_표시:
                ...
                # 미국주식인데 FA가 빈 경우 → yfinance .info 조회 실패 (401 등)
