...
def analyze_one(종목, settings):
    ...
    # ── 진입 추천가 + 목표/손절 (v5.25: 한곳에서 일관 계산) ──────
    # [기존 치명 결함] 목표·손절은 여기서 '현재가' 기준으로, 진입 추천가는
    # 리포트 조립부에서 '현재가-ATR' 기준으로 따로 계산됐다. 급락으로 ATR이
    # 폭증하면 진입가가 손절가보다 아래로 내려가 '사자마자 손절' 모순 발생.
    # → 진입 추천가를 먼저 정하고, 목표·손절을 그 진입가 기준으로 계산한다.
    TARGET = settings.get("TARGET_PROFIT", 25)
    STOP   = settings.get("STOP_LOSS",    -7)

    # ATR 상한 가드: 급락장에서 ATR이 주가의 수십 %가 되면 체결 불가능한
    # 지정가가 나온다. 진입가 괴리를 ENTRY_GAP_MAX_PCT(기본 5%)로 제한.
    _gap_cap = abs(float(settings.get("ENTRY_GAP_MAX_PCT", 5.0))) / 100
    _atr_val = atr if atr and atr > 0 else 0
    _bb_하단 = 결과.get("bb_하단", 0) or 0

    진입가_bb  = _bb_하단 if _bb_하단 > 0 else 0
    진입가_atr = (현재가 - _atr_val) if _atr_val > 0 else 0
    _후보 = [v for v in (진입가_bb, 진입가_atr) if 0 < v < 현재가]
    추천_진입가 = max(_후보) if _후보 else 현재가
    # 괴리 상한 적용 (너무 낮은 지정가 방지 → 실제 체결 가능한 수준 유지)
    추천_진입가 = max(추천_진입가, 현재가 * (1 - _gap_cap))
    _가드적용 = bool(_후보 and max(_후보) < 현재가 * (1 - _gap_cap))

    # 목표·손절은 '진입 추천가' 기준 (진입가 < 손절가 모순 원천 차단)
    목표가 = 추천_진입가 * (1 + TARGET / 100)
    손절가 = 추천_진입가 * (1 + STOP   / 100)
...
def build_report_sections(종목목록, settings, 크립토_하락_레짐, 포트폴리오=None):
    ...
    장세 = classify_market_regime(_장세입력, current_vix=settings.get("CURRENT_VIX"), settings=settings)
    ...
    for rank, (ticker, 우선순위점수, 우선순위세부) in enumerate(신호_종목_순서, 1):
        ...
        신호_종목_요약.append({
            ...
            "현재가":  d["현재가"],
            "market":  market,
            "목표가":  d["목표가"],   # v5.20: 성과추적 채점 정확도용 (근사 대신 실제값)
            "손절가":  d["손절가"],
            # v5.25: 목표·손절이 '진입 추천가' 기준이므로 진입가도 같이 기록해야
            # 채점 기준이 일치한다 (현재가로 채점하면 승률이 왜곡됨)
            "진입가":  d.get("추천_진입가", d["현재가"]),
        })
        ...
        축_표시 = (
            ...
            f"거래량{'✅' if 수급_충족 else '❌'} "   # v5.25: 실제 판정 기준이 거래량 급증
        ...
        # [v5.25] 분할 목표도 '진입 추천가' 기준 — 손절과 기준을 일치시킨다.
        # (기존: 1·2차는 현재가 기준, 손절은 진입가 기준 → 기준 혼재)
        _진입기준 = d.get("추천_진입가") or 현재가
        목표가_1 = _진입기준 * (1 + TARGET_1 / 100)
        목표가_2 = _진입기준 * (1 + TARGET_2 / 100)
        신호_블록 = 신호_블록.replace(
            f"  • 🎯 목표가: {통화_기호}{목표가:,.0f} (+{TARGET:.0f}%) | "
            f"손절가: {통화_기호}{손절가:,.0f} ({STOP:.0f}%)\n",
        ...
        # [v5.25] 계산은 analyze_one에서 이미 끝났다 (목표·손절과 기준 통일).
        # 여기서는 표시만 한다 → 진입가/손절가 기준 불일치 원천 차단.
        추천_진입가 = d.get("추천_진입가", 현재가)
        진입가_bb   = d.get("진입가_bb", 0)
        진입가_atr  = d.get("진입가_atr", 0)
        if 추천_진입가 and 추천_진입가 > 0:
            진입가_표시 = f"{통화_기호}{추천_진입가:,.0f}"
            괴리율 = (추천_진입가 - 현재가) / 현재가 * 100 if 현재가 else 0
            if abs(괴리율) < 0.5:
                진입_메모 = "현재가 근처 (즉시 진입 가능)"
            else:
                진입_메모 = f"현재가 대비 {괴리율:+.1f}% (지정가 대기)"
            신호_블록 += f"  • 📌 진입 추천가: {진입가_표시} ({진입_메모})\n"
            _근거 = []
            if 진입가_bb > 0:
                _근거.append(f"볼린저하단 {통화_기호}{진입가_bb:,.0f}")
            if 진입가_atr > 0:
                _근거.append(f"현재가-ATR {통화_기호}{진입가_atr:,.0f}")
            if _근거:
                신호_블록 += f"     └ {' | '.join(_근거)}\n"
            # 변동성 급증으로 괴리 상한이 걸린 경우 명시 (사용자 혼란 방지)
            if d.get("진입_가드"):
                신호_블록 += (
                    f"     ⚠️ 변동성 급증(ATR 과대) — 진입 괴리를 "
                    f"{abs(괴리율):.1f}%로 제한했습니다\n"
                )
        # 갭업/갭다운 경고
        if abs(갭_비율) >= 2.0:
        ...
        if market == "KR" and 수급 and not 수급.get("데이터_없음"):
            ...
            # [v5.25] 특징 패턴이 없어도 실측치를 항상 표시한다.
            # 기존엔 표시문구가 비면 라인 자체가 사라져, 수급 데이터가
            # 정상 수집됐는데도(진단 29/29) 리포트엔 흔적이 없어 혼란스러웠다.
            if 수급.get("데이터_없음"):
                신호_블록 += "  • ➡️ 외국인/기관 수급: 데이터 없음\n"
            elif 수급_문구:
                신호_블록 += f"  • {수급_아이콘} 외국인/기관 수급: {수급_문구}\n"
            else:
                def _연속_표기(n: int) -> str:
                    if n > 0:
                        return f"{n}일 연속매수"
                    if n < 0:
                        return f"{abs(n)}일 연속매도"
                    return "중립"
                신호_블록 += (
                    f"  • {수급_아이콘} 외국인/기관 수급: "
                    f"외국인 {_연속_표기(외국인_연속)} / "
                    f"기관 {_연속_표기(기관_연속)} (특이 신호 없음)\n"
                )
        elif market == "KR" and (not 수급 or 수급.get("데이터_없음")):
            신호_블록 += "  • ➡️ 수급: 데이터 조회 실패 (KRX/네이버 API)\n"

        # [v5.25] AI가 "거래량" 같은 한 단어만 반환하면 정보가 없으므로 생략.
        # (모델 응답이 잘리거나 부실할 때 리포트에 의미 없는 줄이 남던 문제)
        if ai_이유 and len(str(ai_이유).strip()) >= 10:
            신호_블록 += f"  • 🤖 {ai_이유}\n"
        신호_블록 += bt_문구
    ...
    # v5.21: 모멘텀_결과_맵도 반환 — run_analysis의 성과 추적이 사용
    # (기존 스코프 버그: run_analysis에서 이 지역변수를 참조해 NameError)
    return (매수신호_섹션, 크립토_섹션, 매도신호_섹션,
            신호_종목_요약, 모멘텀_섹션, 장세, 모멘텀_결과_맵)


def _build_momentum_section(모멘텀_결과_맵, 장세, settings, backtest_momentum, 안정신호수: int = 0) -> str:
    """
    ...
    # ── 합산 리스크 관리 (v5.20): 안정+모멘텀 총 포지션 제한 ──────
    # 각 포지션이 총자산의 1% 리스크 → "안정적" 목표 위해 총 리스크 상한.
    # MAX_POSITIONS(기본 8)에서 이미 뜬 안정 신호 수를 빼고, 남은 만큼만 모멘텀 허용.
    # 또한 장세가 K자양극화가 아니면 모멘텀을 더 보수적으로 캡.
    _max_total = int(settings.get("MAX_POSITIONS", 8))
    _남은자리 = max(0, _max_total - 안정신호수)
    # 장세별 모멘텀 상한: K자=최대 활용, 추세장=보조(적게), 그 외=최소
    if 장세["유형"] == "K자양극화":
        _장세캡 = 5
    elif 장세["유형"] == "추세장":
        _장세캡 = 2   # 추세장은 안정이 주력 → 모멘텀 보조
    else:
        _장세캡 = 1
    _모멘텀상한 = max(0, min(_남은자리, _장세캡, 5))

    if _모멘텀상한 == 0:
        return (배너 +
                f"\n⚡ *모멘텀 신호 — 보류* ⚠️고위험·참고용\n"
                f"  안정 신호({안정신호수}개)로 포지션 한도가 찼습니다. "
                f"합산 리스크 관리를 위해 모멘텀은 이번에 보류합니다.\n")

    _전체검증수 = len(검증된)
    검증된 = 검증된[:_모멘텀상한]   # 상한 적용
    ...
    메달 = ["🥇", "🥈", "🥉"] + ["•"] * 10
    for i, (t, r, bt, _) in enumerate(검증된):
...
def run_analysis(include_crypto=True, include_markets=None):
    ...
    (매수신호_섹션, 크립토_섹션, 매도신호_섹션, 신호_종목_요약,
     모멘텀_섹션, 장세, 모멘텀_결과_맵) = build_report_sections(
    ...
    try:
        ...
        if _채점["채점"] > 0:
            ...
            # 목표/손절 도달 실시간 알림을 리포트(텔레그램)에 추가 (v5.20)
            _알림텍스트 = _pt.format_grading_alerts(_채점)
            if _알림텍스트:
                리포트 += "\n" + _알림텍스트 + "─────────────────────────\n"
