...
def grade_pending_signals(가격조회함수) -> dict:
    ...
    for sig in data["signals"]:
        ...
        # ── [v5.25] 체결 여부 확인 ────────────────────────────
        # 진입가는 '지정가 추천'(현재가 이하)이다. 주가가 그 아래로
        # 내려온 적이 없으면 애초에 매수가 체결되지 않았으므로,
        # 승/패로 채점하면 승률이 왜곡된다 → '미체결'로 분류하고 제외.
        미체결 = 이후_저 > 진입
        if 미체결:
            if 상한도달:
                sig["상태"]   = "미체결"
...
def get_performance_summary(최근일수: int = 90) -> dict:
    ...
    for sig in data["signals"]:
        ...
        if sig["상태"] == "보유중":
            결과["보유중"] += 1
            continue
        if sig["상태"] == "미체결":      # v5.25: 지정가 미도달 → 승패 제외
            결과["미체결"] += 1
            continue
...
def format_performance_line(summary: dict) -> str:
    ...
    줄.append(f"  📌 보유중(미채점) {summary['보유중']}건"
              + (f" | 미체결 {summary['미체결']}건(지정가 미도달)"
                 if summary.get("미체결") else ""))
    return "\n".join(줄) + "\n"
...
def record_and_grade(신호_종목_요약: list, 모멘텀_결과_맵: dict,
    ...
    안정_기록 = [{
        ...
        "market": s.get("market", ""),
        "진입가": s.get("진입가") or s.get("현재가", 0),   # v5.25: 추천 진입가 우선
