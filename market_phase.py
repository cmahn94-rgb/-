...
# ═══════════════════════════════════════════════════════════════
# 시장 폭(breadth) 기반 장세 판단 — 듀얼 전략 가중치 결정용
# K자 양극화 / 추세장 / 횡보 / 하락을 분류해서
...
def classify_market_regime(분석결과목록: list, current_vix: float | None = None,
    ...
    [중학생 설명]
    지수만 보면 "오른다"지만, 실제로 오른 종목이 몇 개인지(breadth)를 보면
    장의 진짜 체력이 보인다. 삼성·하이닉스만 오르고 나머지가 다 빠지면
    그게 'K자 양극화'다. 이걸 자동 판별해서 어떤 전략을 쓸지 정한다.

    추가 네트워크 호출 없음 — 이미 분석한 결과 리스트만 집계한다.

    Args:
        분석결과목록: analyze_one 결과 dict들의 리스트 (당일상승률 등 포함)
        current_vix:  settings의 VIX 값

    반환:
      {
        "유형": "추세장"|"K자양극화"|"횡보장"|"하락장",
        "breadth": float,        # 상승 종목 비율 %
        "상승종목": int, "하락종목": int, "전체": int,
        "vix": float,
        "안정_가중": "주력"|"보조"|"침묵"|"현금",
        "모멘텀_가중": "주력"|"보조"|"끔",
        "한줄진단": str,
    ...
    """
    vix = float(current_vix) if current_vix else 0.0

    # breadth 집계: 당일 상승/하락 종목 수
    상승, 하락, 전체 = 0, 0, 0
    급등 = 0  # +3% 이상 급등 종목 (모멘텀 후보)
    절대변동_합 = 0.0
    for r in 분석결과목록:
        if not r:
            continue
    ...
    # 평균 절대변동률 — 횡보장 판별용 (변동이 작으면 방향성 없음)
    평균절대변동 = (절대변동_합 / 전체) if 전체 > 0 else 0
    ...
    # ── 장세 분류 ──────────────────────────────────────────
    # breadth 낮음(소수만 상승) + 급등 종목 존재 = K자 양극화
    # breadth 균형 = 추세장
    # 변동 거의 없음 = 횡보
    # 하락 압도 + VIX 높음 = 하락장
    급등비율 = (급등 / 전체 * 100) if 전체 > 0 else 0

    # 평균 변동이 매우 작으면(±1% 미만) 방향 없는 횡보 우선
    if 평균절대변동 < 1.0 and 급등 == 0:
        유형 = "횡보장"
        안정, 모멘텀 = "보조", "끔"
        진단 = (f"방향성 약함(평균변동 {평균절대변동:.1f}%). "
                f"가짜 돌파 많아 모멘텀 비활성, 선별 대응.")
        return {
            "유형": 유형, "breadth": round(breadth, 1),
            "상승종목": 상승, "하락종목": 하락, "전체": 전체,
            "급등종목": 급등, "vix": round(vix, 1),
    ...
    if breadth < _K_BREADTH and 급등 >= 1 and vix >= _K_VIX:
        ...
        유형 = "횡보장"
        안정, 모멘텀 = "보조", "끔"
        진단 = f"방향성 약함(breadth {breadth:.0f}%). 가짜 돌파 많아 모멘텀 비활성, 선별 대응."
